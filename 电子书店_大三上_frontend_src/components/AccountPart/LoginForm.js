...
class SubmitForm extends React.Component{
    ...
    Check = () => {//转化为异步操作
        ...
        headers.set('Authorization', 'Basic ' + this.str2ab(user + ":" + password).toString('base64'));
        console.log("headers=", headers);
        //let url = "http://localhost:8080/checkUser?username=" + user + "&password=" + password;
        let url = BaseUrl + "login?username=" + user + "&password=" + password;
            ...
            .then(data => {
                ...
                if (data.status >= 0) {
                    ...
                    if (data.data.type === 2) {
                        ...
                        if (data.data.type !== 1) history.push({pathname: "/", state: {isAuthed: true}});
