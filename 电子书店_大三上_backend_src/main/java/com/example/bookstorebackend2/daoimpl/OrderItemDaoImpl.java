...
import com.example.bookstorebackend2.entity.*;
import com.example.bookstorebackend2.repository.*;
...
public class OrderItemDaoImpl implements OrderItemDao {
    ...
    public void addOrderItemsByOrder(List<Cart> items)
    {
        int max = orderRepository.getLastOrder().getOrderId();
        for(Cart cartItem : items){
            ...
            item.setPrice(price);//设置购买时的价格信息(快照)
            item.setBuyNum(num);
        ...
        cartRepository.deleteAllInBatch(items);//删除所有与此订单相关的购物车记录
        //int res = 10 / 0;
    }
}
